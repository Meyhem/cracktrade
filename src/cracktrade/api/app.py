...
Routes are declared ``def`` rather than ``async def`` on purpose (spec section 15.3, D-13):
...
def _problem(error: ApiError, request: Request) -> JSONResponse:
    ...
    return JSONResponse(status_code=int(error.status), content=body, media_type=PROBLEM_JSON)
...
def create_app(settings: ApiSettings | None = None) -> FastAPI:
    ...
    async def _handle_api_error(request: Request, exc: Exception) -> JSONResponse:
        ...
        if isinstance(exc, InvariantViolationError):
            ...
            logger.error("invariant violation on %s: %s", request.url.path, exc.detail)
